// public/apps/botiquin/js/pdf-generator.js
// Generador Oficial Corporativo de PDF HSEQ-F-035 (Inspección de Botiquín de Primeros Auxilios) para Trans Services A&B S.A.S.
...
async function generateBotiquinPDFReport(data) {
  ...
  // 3. TABLA DE LOS 21 ELEMENTOS NORMATIVOS EN 2 COLUMNAS
  const checklist = Array.isArray(data.checklist) ? data.checklist : [];
  const getRecordItem = (id) => {
  ...
  const tableColWidth = (contentWidth - 3) / 2;
  const tableTopY = currentY;

  const renderTableColumn = (startX, itemsIds, headerTitle) => {
    let y = tableTopY;
    ...
    itemsIds.forEach((itemId, idx) => {
      ...
      // Cantidad Encontrada
      const cant = item.cantidad != null ? item.cantidad : (item.req || 1);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(...PALETTE.textMain);
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(5.2);
      const venceStr = item.fechaVencimiento ? item.fechaVencimiento : (item.tieneVencimiento || item.venc ? "PEND" : "N/A");
  ...
  // Renderizar 2 columnas de datos
...
// Exponer globalmente
window.generateBotiquinPDFReport = generateBotiquinPDFReport;
