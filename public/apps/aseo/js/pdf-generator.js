// public/apps/aseo/js/pdf-generator.js
...
async function generateAseoPDFReport(data) {
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...PALETTE.textMain);
  doc.text(`${data.kilometraje ? data.kilometraje.toLocaleString() : "—"} KM`, marginX + 30, metaY1 + metaLineH * 2);
  ...
  // 3. TABLA DE 24 ÍTEMS EN 2 COLUMNAS
  const checklist = Array.isArray(data.checklist) ? data.checklist : [];
  const tableColWidth = (contentWidth - 3) / 2;
  const tableTopY = currentY;

  const renderAseoColumn = (startX, startIndex, count, headerTitle) => {
    let y = tableTopY;
...
window.generateAseoPDFReport = generateAseoPDFReport;
