// lib/utils/pdfExtintorGenerator.ts
...
export interface ExtintorChecklistItemDto {
  id?: number;
  item: string;
  estado: "SI" | "NO" | "NA";
}

export interface ExtintorPdfData {
  id?: string;
  fecha: string;
  hora: string;
  placa: string;
  ...
  checklist: ExtintorChecklistItemDto[];
...
export async function generateExtintorPDF(data: ExtintorPdfData): Promise<void> {
  ...
  CRITERIOS_BASE.forEach((crit, idx) => {
    ...
    // Badges / Marcadores de estado (SI / NO / NA)
